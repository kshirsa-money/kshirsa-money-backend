...
@Tag(name = "7. Budget Crud Controller", description = "APIs for adding money tracking data")
public class BudgetController extends BaseController {

    private final BudgetCrudService budgetCrudService;
    ...
    public ResponseEntity<SuccessResponse<BudgetSegment>> addBudgetSegment(@RequestBody AddBudgetSegmentDto addBudgetSegmentDto) {
        return ok(budgetCrudService.addBudgetSegment(addBudgetSegmentDto), env.getProperty("BUDGET.SEGMENT.ADDED"));
    }
    ...
    public ResponseEntity<SuccessResponse<BudgetSegment>> getBudgetSegment(@PathVariable String segmentId) {
        return ok(budgetCrudService.getBudgetSegment(segmentId), env.getProperty("BUDGET.SEGMENT.RETRIEVED"));
    }
    ...
    public ResponseEntity<SuccessResponse<BudgetSegment>> updateBudgetSegment(@RequestBody UpdateBudgetDto updateSegmentDto) {
        return ok(budgetCrudService.updateBudgetSegment(updateSegmentDto), env.getProperty("BUDGET.SEGMENT.UPDATED"));
    }
    ...
    public ResponseEntity<SuccessResponse<Boolean>> deleteBudgetSegment(@PathVariable String segmentId) {
        return ok(budgetCrudService.deleteBudgetSegment(segmentId), env.getProperty("BUDGET.SEGMENT.DELETED"));
    }
}